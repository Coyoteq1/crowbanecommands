...
namespace CrowbaneCommands.Services
{
    internal class LocalizationService
    {
        ...
        {
            var resourceName = "CrowbaneCommands.Data.English.json";
        ...
        {
            var resourceName = "CrowbaneCommands.Data.PrefabNames.json";
            ...
            if (stream != null)
            {
                ...
                {
                    string jsonContent = reader.ReadToEnd();
                    var rawPrefabNames = JsonSerializer.Deserialize<Dictionary<string, string>>(jsonContent);

                    prefabNames.Clear();

                    if (rawPrefabNames != null)
                    {
                        foreach (var entry in rawPrefabNames)
                        {
                            if (!int.TryParse(entry.Key, NumberStyles.Integer, CultureInfo.InvariantCulture, out var prefabId))
                            {
                                continue; // Skip entries that are not numeric identifiers (e.g., signatures)
                            }

                            prefabNames[prefabId] = entry.Value;
                        }
                    }
                }
            }
